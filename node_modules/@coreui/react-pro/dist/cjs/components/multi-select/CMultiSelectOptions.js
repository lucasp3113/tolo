...
var CMultiSelectOptions = React.forwardRef(function (_a, ref) {
    ...
    var handleKeyDown = function (event, option) {
        ...
        if (event.key === 'Down' || event.key === 'ArrowDown') {
            ...
            var next = getNextSibling.getNextSibling(target, '.form-multi-select-option');
            next && next.focus();
        }
        if (event.key === 'Up' || event.key === 'ArrowUp') {
            event.preventDefault();
            var target = event.target;
            var prev = getPreviousSibling.default(target, '.form-multi-select-option');
            prev && prev.focus();
        }
    };
    var createOptions = function (options) {
        return options.length > 0 ? (options.map(function (option, index$1) {
            return 'value' in option ? (React.createElement("div", { className: index.default('form-multi-select-option', {
                ...
                }), key: index$1, onClick: function () { return onOptionOnClick && onOptionOnClick(option); }, onKeyDown: function (event) { return handleKeyDown(event, option); }, tabIndex: 0, role: "option", "aria-selected": selected.some(function (_option) { return _option.value === option.value; }), "aria-disabled": Boolean(option.disabled) }, optionsTemplate ? optionsTemplate(option) : option.label)) : (React.createElement("div", { className: "form-multi-select-optgroup-label", key: index$1 }, optionsGroupsTemplate ? optionsGroupsTemplate(option) : option.label));
