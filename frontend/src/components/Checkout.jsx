...
function CheckoutBricks({ total, onPaymentSuccess, idCompra, data, toloCoinsGanados, toloCoinsRestados, userId }) {
    ...
    useEffect(() => {
        ...
        if (!window.MercadoPago) {
            setError('MercadoPago SDK no está cargado. Verifica que el script esté en el HTML.');
            setLoading(false);
            return;
        }
    ...
    return (
        <div className='font-semibold mercado_pago translate-y-8 md:traslate-y-2 lg:translate-y-2' style={{
            maxWidth: '600px',
            margin: 'auto',
            padding: '20px',
            ...
            {loading && (
                <div style={{
                    ...
                    <p style={{ fontFamily: 'Quicksand, sans-serif' }}>
                        {preferenceId ? 'Cargando formulario de pago...' : 'Preparando preferencia de pago...'}
                    </p>
                </div>
            )}
